...
The system employs email-based authentication and a robust role-based access control mechanism supporting admin, manager, partner, broker, customer, and rep roles. User statuses include pending, active, denied, paused, and cancelled. Session state is managed client-side via an AuthContext. Manager and partner permissions are configurable by admins, controlling access to features like lead viewing, assignment, broker management, credit adjustments, and settings.

### Partner Role
Partners are a distinct user type with the same configurable permission system as managers. They can log in to the CRM and access features based on their assigned permissions (same permission flags as managers). Partner accounts are created via the Manager tab ("Add New User" → select "Partner" role) and listed in a dedicated Partner Accounts section within the Manager tab. Both the `hasPermission` helper (frontend) and `checkPermission` function (backend) treat partner role identically to manager for permission evaluation. The Partner role login option appears on the Login page.
