...
export default function DocSign() {
  ...
  const handleSubmit = async () => {
    if (isEmpty) { alert("Please draw your signature before submitting."); return; }
  ...
  const isPdf = record?.documentMimeType === "application/pdf" || record?.documentPath?.endsWith(".pdf");
  ...
  if (record?.status === "signed") return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="bg-white rounded-xl shadow p-8 max-w-md text-center">
        ...
        <p className="text-gray-600">This document was already signed by <strong>{record.signerName}</strong>.</p>
        {record.signedAt && (
          <p className="text-gray-400 text-sm mt-1">
            Signed on {new Date(record.signedAt).toLocaleDateString("en-CA", { year: "numeric", month: "long", day: "numeric" })}
          </p>
        )}
      </div>
    </div>
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-blue-700 text-white rounded-t-xl p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold">Document Signing Request</h1>
              {record?.propertyAddress && (
                <p className="text-blue-200 text-sm mt-0.5">{record.propertyAddress}</p>
              )}
            </div>
          </div>
        </div>
        ...
        {/* Document Info */}
        <div className="bg-white border-x border-gray-200 px-6 py-4 flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-gray-800">{record?.documentName || "Uploaded Document"}</p>
            <p className="text-xs text-gray-400">Please review the document below before signing</p>
          </div>
          {record?.documentPath && (
            <a
        ...
        {/* Signature Section */}
        <div className="bg-white border border-gray-200 rounded-b-xl p-6 border-t-0">
          ...
          <div className="grid gap-5">
            ...
            <div>
              ...
              {isEmpty && <p className="text-xs text-gray-400 mt-1">Draw your signature in the box above</p>}
            </div>
            ...
            <div className="pt-2">
              ...
              <p className="text-xs text-gray-400 mt-2">
                By submitting, you confirm that you have reviewed and agree to the contents of this document.
              </p>
            </div>
          </div>
        </div>
        ...
        <p className="text-center text-xs text-gray-400 mt-6">
          Powered by QuoteUs.ca &mdash; Secure document signing
        </p>
      </div>
    </div>
  );
