...
export default function ServiceSign() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/service-sign/${token}`)
      ...
      .then(data => {
        const { attachments: atts, ...sa } = data;
        setAgreement(sa);
        setAttachments(atts || []);
        if (sa.status === "signed") setSubmitted(true);
      })
          ...
          {attachments.length > 0 && (
            <div className="border border-gray-200 rounded-xl overflow-hidden">
              <div className="bg-gray-50 px-4 py-2.5 border-b">
                <p className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Additional Documents ({attachments.length})</p>
              </div>
